...
$url = new moodle_url('/mod/evaluation/import.php', array('id' => $id, 'clid' => $classid));
if ($choosefile !== false) {
...
if ($choosefile) {
    ...
    if (!$xmldata = evaluation_load_xml_data($xmlcontent)) {
        print_error('cannotloadxml', 'evaluation', 'edit.php?id=' . $id . '&clid=' . $classid . '');
    }

    $importerror = evaluation_import_loaded_data($xmldata, $evaluation->id);
    if ($importerror->stat == true) {
        $url = 'edit.php?id=' . $id . '&clid=' . $classid . '&do_show=templates';
        redirect($url, get_string('import_successfully', 'local_evaluations'), 3);
        exit;
    }
}
...
///////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
echo $OUTPUT->heading(get_string('import_questions', 'local_evaluations'));
...
function evaluation_import_loaded_data(&$data, $evaluationid) {
    ...
    if (!is_array($data)) {
        $error->msg[] = get_string('data_is_not_an_array', 'local_evaluations');
        $error->stat = false;
        return $error;
    }
