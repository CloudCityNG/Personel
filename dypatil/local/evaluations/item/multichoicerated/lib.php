...
require_once($CFG->dirroot.'/local/evaluations/item/evaluation_item_class.php');
...
class evaluation_item_multichoicerated extends evaluation_item_base {
    ...
    public function print_analysed($item, $itemnr = '', $groupid = false, $courseid = false) {
        global $OUTPUT;
        $sep_dec = get_string('separator_decimal', 'local_evaluations');
        if (substr($sep_dec, 0, 2) == '[[') {
            $sep_dec = EVALUATION_DECIMAL;
        }

        $sep_thous = get_string('separator_thousand', 'local_evaluations');
        if (substr($sep_thous, 0, 2) == '[[') {
            $sep_thous = EVALUATION_THOUSAND;
        }
        ...
        if ($analysed_item) {
            ...
            foreach ($analysed_vals as $val) {
                $intvalue = $pixnr % 10;
                $pix = $OUTPUT->pix_url('multichoice/' . $intvalue, 'local_evaluations');
                $pixnr++;
                $pixwidth = intval($val->quotient * EVALUATION_MAX_PIX_LENGTH);
                ...
                echo '<td align="left" style="width: '.EVALUATION_MAX_PIX_LENGTH.'">';
            ...
            echo get_string('average', 'local_evaluations').': '.$avg.'</b>';
            echo '</td></tr>';
        }
    }
    ...
    public function excelprint_item(&$worksheet, $row_offset,
        ...
        if (is_array($data)) {
            ...
            $worksheet->write_string($row_offset,
                                count($data) + 2,
                                get_string('average', 'local_evaluations'),
                                $xls_formats->value_bold);
    ...
    /**
     * print the item at the edit-page of evaluation
     *
     * @global object
     * @param object $item
     * @return void
    ...
    public function print_item_preview($item) {
        ...
        $info = $this->get_info($item);
        $str_required_mark = '<span class="evaluation_required_mark">*</span>';

        $lines = explode (EVALUATION_MULTICHOICERATED_LINE_SEP, $info->presentation);
        $requiredmark =  ($item->required == 1) ? $str_required_mark : '';
        //print the question and label
        echo '<div class="evaluation_item_label_'.$align.'">';
        echo '('.$item->label.') ';
        echo format_text($item->name.$requiredmark, true, false, false);
        if ($item->dependitem) {
            if ($dependitem = $DB->get_record('evaluation_item', array('id'=>$item->dependitem))) {
                echo ' <span class="evaluation_depend">';
        ...
        echo '<div class="evaluation_item_presentation_'.$align.'">';
        switch($info->subtype) {
            case 'r':
    ...
    /**
     * print the item at the complete-page of evaluation
     *
     * @global object
     * @param object $item
     * @param string $value
    ...
    public function print_item_complete($item, $value = '', $highlightrequire = false) {
        ...
        //print the question and label
        echo '<div class="evaluation_item_label_'.$align.$highlight.'">';
            echo format_text($item->name.$requiredmark, true, false, false);
        echo '</div>';

        //print the presentation
        echo '<div class="evaluation_item_presentation_'.$align.$highlight.'">';
        switch($info->subtype) {
            case 'r':
    ...
    public function print_item_show_value($item, $value = '') {
        ...
        $lines = explode (EVALUATION_MULTICHOICERATED_LINE_SEP, $info->presentation);
        $requiredmark = ($item->required == 1)?'<span class="evaluation_required_mark">*</span>':'';
        ...
        foreach ($lines as $line) {
            if ($value == $index) {
                $item_value = explode(EVALUATION_MULTICHOICERATED_VALUE_SEP, $line);
    ...
    public function get_presentation($data) {
        ...
        if (isset($data->horizontal) AND $data->horizontal == 1 AND $subtype != 'd') {
            $present .= EVALUATION_MULTICHOICERATED_ADJUST_SEP.'1';
        }
        return $subtype.EVALUATION_MULTICHOICERATED_TYPE_SEP.$present;
    }
    ...
    private function print_item_radio($item, $value, $info, $align, $showrating, $lines) {
        ...
        if (!$this->hidenoselect($item)) {
            ?>
                <li class="evaluation_item_radio_<?php echo $hv.'_'.$align;?>">
                    <span class="evaluation_item_radio_<?php echo $hv.'_'.$align;?>">
                        <?php
                    ...
                    <span class="evaluation_item_radiolabel_<?php echo $hv.'_'.$align;?>">
                        <label for="<?php echo $item->typ . '_' . $item->id.'_xxx';?>">
                        <?php print_string('not_selected', 'local_evaluations');?>&nbsp;
                        </label>
                    </span>
                </li>
            <?php
        ...
        foreach ($lines as $line) {
            ...
            $radio_value = explode(EVALUATION_MULTICHOICERATED_VALUE_SEP, $line);
    ...
    private function print_item_dropdown($item, $value, $info, $align, $showrating, $lines) {
        ...
        echo '<ul>';
        ?>
        <li class="evaluation_item_select_<?php echo $hv.'_'.$align;?>">
            ...
            <select id="<?php echo $item->typ.'_'.$item->id;?>" name="<?php echo $item->typ.'_'.$item->id;?>">
                ...
                foreach ($lines as $line) {
                    ...
                    $dropdown_value = explode(EVALUATION_MULTICHOICERATED_VALUE_SEP, $line);
                    if ($showrating) {
