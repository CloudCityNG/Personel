...
class evaluation_item_multichoice extends evaluation_item_base {
    ...
    public function print_analysed($item, $itemnr = '', $groupid = false, $courseid = false) {
        ...
        if ($analysed_item) {
            ...
            foreach ($analysed_vals as $val) {
                $intvalue = $pixnr % 10;
                $pix = $OUTPUT->pix_url('multichoice/' . $intvalue, 'evaluation');
                $pixnr++;
                ...
                echo '<td align="left" valign="top">
                      ...
                      <td align="left" style="width:'.EVALUATION_MAX_PIX_LENGTH.';">
    ...
    public function print_item_complete($item, $value = null, $highlightrequire = false) {
        ...
        // if ($info->subtype == 'c') {
            if (is_array($value)) {
                ...
                $values = explode(EVALUATION_MULTICHOICE_LINE_SEP, $value);
            }
            $highlight = '';
    ...
    public function print_item_show_value($item, $value = null) {
        ...
        $requiredmark = '';
        if ($item->required == 1) {
            $requiredmark = '<span class="evaluation_required_mark">*</span>';
        }
        ...
        if ($info->subtype == 'c') {
            ...
            foreach ($presentation as $pres) {
                foreach ($values as $val) {
                    if ($val == $index) {
                        echo '<div class="evaluation_item_multianswer">';
    ...
    public function get_presentation($data) {
        $present = str_replace("\n", EVALUATION_MULTICHOICE_LINE_SEP, trim($data->itemvalues));
        ...
        if (isset($data->horizontal) AND $data->horizontal == 1 AND $subtype != 'd') {
            $present .= EVALUATION_MULTICHOICE_ADJUST_SEP.'1';
        }
        return $subtype.EVALUATION_MULTICHOICE_TYPE_SEP.$present;
    }
    ...
    private function item_array_to_string($value) {
        ...
        for ($i = 1; $i < $sizeofarrvals; $i++) {
            $retval .= EVALUATION_MULTICHOICE_LINE_SEP.$arrvals[$i];
        }
        return $retval;
    }
    ...
    private function print_item_check($presentation, $item, $value, $info, $align) {
        ...
        ?>
            <li class="evaluation_item_check_<?php echo $hv.'_'.$align;?>">
                <span class="evaluation_item_check_<?php echo $hv.'_'.$align;?>">
                    <?php
