...
class evaluation_item_numeric extends evaluation_item_base {
    ...
    public function init() {
        $this->sep_dec = get_string('separator_decimal', 'local_evaluations');
        if (substr($this->sep_dec, 0, 2) == '[[') {
            $this->sep_dec = EVALUATION_DECIMAL;
        }

        $this->sep_thous = get_string('separator_thousand', 'local_evaluations');
        if (substr($this->sep_thous, 0, 2) == '[[') {
            $this->sep_thous = EVALUATION_THOUSAND;
        }
    }
    ...
    public function print_analysed($item, $itemnr = '', $groupid = false, $courseid = false) {
        ...
        if (isset($values->data) AND is_array($values->data)) {
            ...
            echo get_string('average', 'local_evaluations').': '.$avg;
            echo '</b></td></tr>';
        }
    }
    ...
    public function print_item_preview($item) {
        ...
        if ($item->dependitem) {
            ...
            if ($dependitem = $DB->get_record('evaluation_item', $params)) {
                echo ' <span class="evaluation_depend">';
                echo '('.$dependitem->label.'-&gt;'.$item->dependvalue.')';
                echo '</span>';
            }
        }
        echo '<span class="evaluation_item_numinfo">';
        switch(true) {
            case ($range_from === '-' AND is_numeric($range_to)):
                echo ' ('.get_string('maximal', 'local_evaluations').
                        ': '.str_replace(EVALUATION_DECIMAL, $this->sep_dec, $range_to).')';
                break;
            case (is_numeric($range_from) AND $range_to === '-'):
                echo ' ('.get_string('minimal', 'local_evaluations').
                        ': '.str_replace(EVALUATION_DECIMAL, $this->sep_dec, $range_from).')';
                break;
            case ($range_from === '-' AND $range_to === '-'):
                break;
            default:
                echo ' ('.str_replace(EVALUATION_DECIMAL, $this->sep_dec, $range_from).
                        ' - '.str_replace(EVALUATION_DECIMAL, $this->sep_dec, $range_to).')';
                break;
        }
        echo '</span>';
        ...
        echo '<span class="evaluation_item_textfield">';
    ...
    public function check_value($value, $item) {
        $value = str_replace($this->sep_dec, EVALUATION_DECIMAL, $value);
    ...
    public function create_value($data) {
        $data = str_replace($this->sep_dec, EVALUATION_DECIMAL, $data);
    ...
    public function get_presentation($data) {
        $num1 = str_replace($this->sep_dec, EVALUATION_DECIMAL, $data->numericrangefrom);
        if (is_numeric($num1)) {
            $num1 = floatval($num1);
        } else {
            $num1 = '-';
        ...
        $num2 = str_replace($this->sep_dec, EVALUATION_DECIMAL, $data->numericrangeto);
        if (is_numeric($num2)) {
            $num2 = floatval($num2);
        } else {
            $num2 = '-';
