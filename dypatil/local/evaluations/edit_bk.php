...
//$evaluation->classid = optional_param('clid', false, PARAM_INT);
...
$url = new moodle_url('/local/evaluation/edit.php', array('id'=>$id, 'do_show'=>$do_show));
//
//if (! $cm = get_classlocalule_from_id('evaluation', $id)) {
//    print_error('invalidclasslocalule');
//}

//if (! $evaluation->classid = $DB->get_record('class', array('id'=>$cm->class))) {
...
//
//$context = context_localule::instance($cm->id);

//require_login($evaluation->classid, true, $cm);
...
//    if (!has_capability('local/evaluation:createprivatetemplate', $context) AND
 ...
 //       if (has_capability('local/evaluation:createpublictemplate', get_system_context())) {
       ...
       // }
        if (!evaluation_save_as_template($evaluation,
...
//Adding the javascript localule for the items dragdrop.
if (count($evaluationitems) > 1) {
    if ($do_show == 'edit' and $CFG->enableajax) {
        ...
        $PAGE->requires->yui_module('moodle-local_evaluations-dragdrop', 'M.local_evaluations.init_dragdrop',
                array(array('cmid' => $evaluation->classid)));
    }
}
...
if ($do_show == 'edit') {
    ...
    if (is_array($evaluationitems)) {
        ...
        if (isset($SESSION->evaluation->moving) AND $SESSION->evaluation->moving->shouldmoving == 1) {
            ...
            $anker .= get_string('cancel_moving', 'local_evaluation');
            $anker .= '</a>';
        ...
        foreach ($evaluationitems as $evaluationitem) {
            ...
            echo $OUTPUT->box_start('evaluation_item_box_'.$align.$dependstyle,
                                    'evaluation_item_box_'.$evaluationitem->id);
            ...
            echo '('.get_string('position', 'local_evaluations').':'.$itempos .')';
            echo '</span>';
            ...
            if ($evaluationitem->typ != 'pagebreak') {
                ...
                // and the cmid is the id to get the localule.
        ...
        echo $OUTPUT->box(get_string('no_items_available_yet', 'lcoal_evaluations'),
