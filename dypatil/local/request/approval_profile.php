...
$PAGE->set_url('/local/request/approval_profile.php');
...
$strheading = get_string('profileapprovals', 'local_request');
...
$data = array();
echo $OUTPUT->header();
$requests = $DB->get_records('local_request_profile_change');
...
foreach ($requests as $request) {
    ...
    if ($vals = $DB->get_records_sql("SELECT * FROM {local_school_permissions} where userid = '" . $USER->id . "' and schoolid='" . $request->schoolid . "'")) {
        ...
        $serviceid = $requestpro->service($request->schoolid, $request->programid, $request->studentid);
        ...
        if ($reg_approval == 0) {
            $buttons[] = html_writer::link(new moodle_url('/local/profilechange/changeprofile.php', array('confirm' => 1, 'recid' => $request->id)), html_writer::empty_tag('img', array('src' => $OUTPUT->pix_url('t/check'), 'title' => get_string('approve', 'local_courseregistration'), 'alt' => get_string('approve', 'local_courseregistration'), 'class' => 'iconsmall')));

            $buttons[] = html_writer::link(new moodle_url('/local/request/reject.php', array('reject' => 2, 'recid' => $request->id)), html_writer::empty_tag('img', array('src' => $OUTPUT->pix_url('t/delete'), 'title' => get_string('reject', 'local_courseregistration'), 'alt' => get_string('reject', 'local_courseregistration'), 'class' => 'iconsmall')));
...
if (isset($CFG->allowframembedding) and ! $CFG->allowframembedding) {
    echo $OUTPUT->box(get_string('profileapprovedes', 'local_request'));
}
echo "<br>";
echo $OUTPUT->box_start('generalbox');
if (!empty($data)) {
    $PAGE->requires->js('/local/request/js/approveprofilejs.js');
