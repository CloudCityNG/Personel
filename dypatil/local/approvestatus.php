    ...
    if ($classList) {
        foreach ($classList as $list) {
            ...
            if ($approveStatus == 1 AND $mentorStatus == 1 AND $registratStatus == 1) {
                $line[] = get_string('confirmed', 'local_courseregistration');
                $line[] = get_string('un-enroll', 'local_courseregistration');
            }
...
//start the table
    ...
    $table->head = array(
        get_string('coursename', 'local_courseregistration'), get_string('code', 'local_clclasses'), get_string('instructor', 'local_courseregistration'), get_string('date', 'local_courseregistration'), get_string('timing', 'local_courseregistration'), get_string('credithours', 'local_clclasses'), get_string('status', 'local_courseregistration'), get_string('unenroll', 'local_courseregistration'));
