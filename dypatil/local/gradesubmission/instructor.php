...
//Setting page url
$PAGE->set_url('/local/gradesubmission/instructor.php');
...
//tabs
createtabview_gsub($currenttab, $inst = 1);
...
if (!empty($_POST)) {
    ...
    $returnurl = new moodle_url('/local/gradesubmission/instructor.php', array('school' => $school, 'semester' => $semester, 'class' => $class));
    $data = $_REQUEST;
    //print_object($data); 
...
if (!empty($users) AND ! empty($exams)) {
    ...
    echo "<form action='instructor.php?school=" . $school . "&semester=" . $semester . "&class=" . $class . "' method='POST'>";
    ...
    $sno = 1;
    /*
     * ###Bugreport #176- Grade submission
     * @author Naveen Kumar<naveen@eabyas.in>
     * (Resolved) Added Javascript validation
     */
    ?>
    ...
    foreach ($users as $user) {
        ...
        foreach ($exams as $exam) {
            ...
            if ($exam->source == 'offline') {
                ...
                /* Bug report #310  -  Instructor>Online activities>Grades- Not displayed
                 * @author hemalatha c arun <hemalatha@eabyas.in>
                 * Resolved-  changed the query string to display online activity marks
                 */
