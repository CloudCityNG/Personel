...
class local_clclasses_renderer extends plugin_renderer_base {

    protected function render_classes(classes $clobject) {
        return $this->class_view($clobject->s_classlist, $clobject);
    }
         ...
         $first_inner ='';
          // to get instructor name
            ...
            if (has_any_capability($visible_cap, $systemcontext)) {
                ...
                $first_inner .='<span class="td_mainitem" id="cl_name">'. $clas->classname .'</span><span id="cl_instructorname" class="item_italic item_gray">  with '. $instructor_info . '</span>';
                ...
                if($onlinecourse)
                $first_inner .= ',   '. get_string('cobaltcourse','local_clclasses').': <b>'. $onlinecourse->fullname.'</b></li>';
                }
