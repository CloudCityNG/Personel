...
class prerequisite_form extends moodleform {
    ...
    function definition_after_data() {
        ...
        if ($countvalue != 1) {
            ...
            if (is_array($schoolvalue) && !empty($schoolvalue) && $schoolvalue[0] != 0) {
                ...
                $dep2 = $mform->createElement('select', 'predeptid', get_string('predept', 'local_cobaltcourses'), $dept);
                $mform->insertElementBefore($dep2, 'addcourselisthere5');
                $mform->addRule('predeptid', get_string('missingdepartment', 'local_cobaltcourses'), 'required', null, 'client');

                $formatvalue1 = $mform->getElementValue('departmentid');
                $formatvalue2 = $mform->getElementValue('predeptid');
            }
        }
        ...
        if (isset($formatvalue3)) {
            if (is_array($formatvalue2) && !empty($formatvalue2) && $formatvalue2[0] != 0) {
                ...
                $newe2 = $mform->createElement('select', 'precourseid', get_string('precourse', 'local_cobaltcourses'), $courses);
                ...
                $mform->addRule('precourseid', get_string('missingcourse', 'local_cobaltcourses'), 'required', null, 'client');
            }
        }
    }
    ...
    public function validation($data, $files) {
        ...
        $nextcourse = $data['precourseid'];
        $courses = array();

        foreach ($nextcourse as $course) {
            $courses[] = $DB->get_record_sql("SELECT * FROM {$CFG->prefix}local_course_prerequisite WHERE courseid = $courseid AND precourseid LIKE '%$course%'");
        }

        $courses = array_filter($courses);
        if (!empty($courses)) {
            $errors['precourseid'] = get_string('prerecordexists', 'local_cobaltcourses');
        }
        ...
        if ($data['predeptid'] == '0') {
            $errors['predeptid'] = 'Select Department Name';
        }
        return $errors;
    }
