...
/**
 * Bulk grades registration functions
 *
 * @package    local
 * @subpackage uploadgrades
 * @copyright  2013 onwards sreenivasulareddy <sreenivasula@eabyas.in>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
defined('MOODLE_INTERNAL') || die();

define('UU_GRADES_ADDNEW', 0);
define('UU_GRADES_ADD_UPDATE', 1);
define('UU_GRADES_UPDATE', 2);
...
/**
 * Tracking of processed grades.
 *
 * This class prints grades information into a html table.
 *
 * @package    core
 * @subpackage admin
 ...
 * @param array $stdfields standard grades fields
 * @param array $profilefields custom profile fields
 * @param moodle_url $returnurl return url in case of any error
 * @return array list of fields
 */
function uu_validate_grades_upload_columns(csv_import_reader $cir, $stdfields, $profilefields, moodle_url $returnurl) {
    ...
    foreach ($columns as $key => $unused) {
        ...
        if (in_array($field, $stdfields) or in_array($lcfield, $stdfields)) {
            ...
            $newfield = $lcfield;
        } else if (preg_match('/^(cohort|grades|group|type|role|enrolperiod)\d+$/', $lcfield)) {
...
function get_inst_school_semesters($id) {
    global $DB, $USER;
    $today = time();
    ...
    $sql = "SELECT ls.id,ls.fullname
                    ...
                    ON ss.semesterid=ls.id where ss.schoolid={$id} AND ls.visible=1 and ls.startdate<{$today} and ls.enddate>{$edate} and ss.instructorid={$USER->id} group by ls.id";
...
function get_inst_assignedschools() {
    global $DB, $CFG, $USER;
    $items = array();
