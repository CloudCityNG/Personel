 ...
 *
 **/
function class_enrol_user($classid, $userid, $classlist = array(), $batchid=0){
    global $DB, $CFG, $USER, $OUTPUT; $batchflag=0;
    ...
    $userdata= $DB->get_record('local_userdata',array('userid'=>$userid, 'schoolid'=>$class->schoolid));
    
    if($batchid == $userdata->batchid && ($batchid!=0)){
        
        $activeplan=$DB->get_record('local_activeplan_batch',array('schoolid'=>$class->schoolid, 'semesterid'=>$class->semesterid,'batchid'=>$batchid));
        $record->planid= $activeplan->planid;
        $record->batchid= $activeplan->batchid;
        $batchflag=1;
    ...
    if(!$exist = $DB->get_record('local_user_clclasses', array('userid'=>$userid, 'classid'=>$classid))){
        ...
        if($exist->registrarapproval == 0 || $exist->registrarapproval == 2 || $exist->registrarapproval == 5){
            ...
            $exist->planid= $activeplan->planid;
            $exist->batchid= $activeplan->batchid;
    ...
    $sem->curriculumid = 0;
    if($batchflag){        
      $sem->planid= $activeplan->planid;
      $sem->batchid= $activeplan->batchid;
    }    
