...
!function (a, b) {
    ...
    function e() {
        ...
        for (var a in l)
            if (l.hasOwnProperty(a))
                for (var b in k[a])
                    if (k[a].hasOwnProperty(b) && k[a][b].matches) {
    ...
    function f(a) {
        ...
        if (b.active || !b.active && a.matches) {
            for (var d in b[c])
                b[c].hasOwnProperty(d) && b[c][d].apply(b.mq);
            b.active = !0
        }
    }
    function g(a, b) {
    ...
    var i = void 0 !== b.matchMedia, j = null, k = {}, l = {minWidth: "min-width", maxWidth: "max-width", minHeight: "min-height", maxHeight: "max-height"}, m = [], n = {minWidth: [0], maxWidth: [1 / 0], minHeight: [0], maxHeight: [1 / 0], unit: "px"}, o = {bind: function (a, c) {
            ...
            for (var d in c)
                c.hasOwnProperty(d) && m[a].hasOwnProperty(d) && m[a][d].push(c[d]);
...
(function () {
    ...
    n.stringify = function (e) {
        ...
        var r = {type: e.type, content: n.stringify(e.content), tag: "span", clclasses: ["token", e.type], attributes: {}};
        r.type == "comment" && (r.attributes.spellcheck = "true");
        t.hooks.run("wrap", r);
        var i = "";
        for (var s in r.attributes)
            i += s + '="' + (r.attributes[s] || "") + '"';
        return"<" + r.tag + ' class="' + r.clclasses.join(" ") + '" ' + i + ">" + r.content + "</" + r.tag + ">"
    };
    if (!self.document) {
...
;
Prism.languages.javascript = Prism.languages.extend("clike", {keyword: /\b(var|let|if|else|while|do|for|return|in|instanceof|function|new|with|typeof|try|catch|finally|null|break|continue)\b/g, number: /\b(-?(0x)?\d*\.?[\da-f]+|NaN|-?Infinity)\b/g});
...
(function () {
    var els = "address|article|aside|audio|canvas|command|datalist|details|dialog|figure|figcaption|footer|header|hgroup|keygen|mark|meter|menu|nav|progress|ruby|section|time|video".split('|');
    for (var i = 0; i < els.length; i++) {
        document.createElement(els[i]);
    }
})();
$(document).ready(function () {
    $.rubberband({minWidth: [320, 500, 740, 980, 1220], maxWidth: [1220, 980, 740, 500, 320]});
    $("code").each(function () {
