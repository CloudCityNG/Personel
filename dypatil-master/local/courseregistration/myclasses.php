...
if ($classList) {
    foreach ($classList as $list) {
        ...
        $line = array();
        $instructor = array();
        $line[] = '<a title="View Class Details" href="' . $CFG->wwwroot . '/local/clclasses/view.php?id=' . $list->classid . '&sesskey=' . sesskey() . '" target="_blank">' . $classinfo->shortname . '</a>';
...
if (has_capability('local/courseregistration:view', $context))
    $table->head = array(get_string('code', 'local_clclasses'), 
        get_string('coursename', 'local_courseregistration'), $ins, get_string('date', 'local_courseregistration'), get_string('timing', 'local_courseregistration'), get_string('credithours', 'local_courseregistration'), get_string('launch', 'local_courseregistration'), get_string('room', 'local_courseregistration'));
else
    $table->head = array(
...
if (has_capability('local/courseregistration:view', $context)) {
    ...
    $activeplan = $DB->get_record('local_activeplan_batch', array('batchid'=>$udata->batchid, 'semesterid'=>$list->semid));
    $plan = $DB->get_record('local_curriculum_plan', array('id'=>$activeplan->planid));
    $heading = html_writer::tag('h5', '<b>' . $plan->fullname . ' (' . $list->sem . ')</b>');
    echo html_writer::tag('div', $heading, array('style'=>'border: 1px solid #E8E8E8; background: #EAEAEA; padding: 0 5px; width: 99%; margin: 25px 0; overflow: auto;'));
}
...
$output = $PAGE->get_renderer('local_courseregistration');


$myclassobject = new local_courseregistration_render();
echo $output->render($myclassobject);
echo $output->footer();
