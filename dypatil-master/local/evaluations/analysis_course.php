...
if (!$cm = get_coursemodule_from_id('local_evaluations', $id)) {
...
if (has_capability('mod/feedback:viewreports', $context)) {
    ...
    $aurl = new moodle_url('analysis_to_excel.php', array('sesskey' => sesskey(),
        'id' => $id,
...
//show the count
echo '<b>' . get_string('completed_feedbacks', 'local_evaluations') . ': ' . $completedscount . '</b><br />';

// get the items of the feedback
$params = array('local_evaluations' => $feedback->id, 'hasvalue' => 1);
...
if (is_array($items)) {
    ...
    echo get_string('show_all', 'local_evaluations');
    echo '</a>';
} else {
    $items = array();
}
...
if ($courseitemfilter > 0) {
    ...
    echo html_writer::label(get_string('search_course', 'local_evaluations') . ': ', 'searchcourse');
    ...
    if ($courses = $DB->get_records_sql_menu($sql, $params)) {
        ...
        echo ' ' . html_writer::label(get_string('filter_by_course', 'local_evaluations'), 'coursefilterid') . ': ';
        echo html_writer::select($courses, 'coursefilter', $coursefilter, null, array('id' => 'coursefilterid', 'class' => 'autosubmit'));

        $PAGE->requires->yui_module('moodle-core-formautosubmit', 'M.core.init_formautosubmit', array(array('selectid' => 'coursefilterid', 'nothing' => false))
        );
    }
    echo '<hr />';
    $itemnr = 0;
    ...
    foreach ($items as $item) {
        ...
        if (preg_match('/rated$/i', $item->typ)) {
            ...
            $anker = '<a href="#" ' . $onclick . '>' .
                    get_string('sort_by_course', 'local_evaluations') .
                    '</a>';
