...
$typ = optional_param('typ', false, PARAM_ALPHA);
 $eid = optional_param('eid', false, PARAM_INT);
 $id = optional_param('id', false, PARAM_INT);
$action = optional_param('action', false, PARAM_ALPHA);

$editurl = new moodle_url('/local/evaluations/edit.php', array('id'=>$id));

if (!$typ) {
    redirect($editurl->out(false));
}

$url = new moodle_url('/local/evaluations/edit_item.php', array('classid'=>$classid,'eid'=>$eid));
if ($typ !== false) {
    $url->param('typ', $typ);
}
if ($id !== false) {
...
if (! $evaluation = $DB->get_record("local_evaluation", array("id"=>$eid))) {
