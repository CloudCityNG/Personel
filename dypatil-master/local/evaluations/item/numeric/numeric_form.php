...
class evaluation_numeric_form extends evaluation_item_form {
    ...
    public function get_data() {
        ...
        $itemobj = new evaluation_item_numeric();

        $num1 = str_replace($itemobj->sep_dec, EVALUATION_DECIMAL, $item->rangefrom);
        if (is_numeric($num1)) {
            $num1 = floatval($num1);
        } else {
            $num1 = '-';
        ...
        $num2 = str_replace($itemobj->sep_dec, EVALUATION_DECIMAL, $item->rangeto);
        if (is_numeric($num2)) {
            $num2 = floatval($num2);
        } else {
            $num2 = '-';
