...
$url = new moodle_url('/local/evaluations/export.php', array('id' => $id, 'clid' => $classid));
if ($action !== false) {
...
redirect('view.php?id=' . $id . '&clid=' . $classid . '');
exit;
...
function evaluation_get_xml_data($evaluationid) {
    ...
    $data .= '<EVALUATION VERSION="200701" COMMENT="XML-Importfile for local/evaluations">' . "\n";
    $data .= $space . '<ITEMS>' . "\n";
