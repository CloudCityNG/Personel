...
$pageparams = new local_att_preferences_page_params();
...
switch ($att->pageparams->action) {
    case local_att_preferences_page_params::ACTION_ADD:
        ...
        break;
    case local_att_preferences_page_params::ACTION_DELETE:
        if (local_att_has_logs_for_status($att->pageparams->statusid)) {
            print_error('cantdeletestatus', 'local_attendance', "attsettings.php?id=$id");
        }

        $confirm    = optional_param('confirm', null, PARAM_INT);
        if (isset($confirm)) {
            $att->remove_status($att->pageparams->statusid);
            redirect($att->url_preferences(), get_string('statusdeleted', 'local_attendance'));
        }

        $statuses = $att->get_statuses();
        $status = $statuses[$att->pageparams->statusid];
        $message = get_string('deletecheckfull', '', get_string('variable', 'local_attendance'));
        $message .= str_repeat(html_writer::empty_tag('br'), 2);
        $message .= $status->acronym.': '.
                    ($status->description ? $status->description : get_string('nodescription', 'local_attendance'));
