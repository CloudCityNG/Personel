...
require_once($CFG->dirroot . '/local/attendance/renderhelpers.php');
//require_once(dirname(__FILE__).'/renderhelpers.php');

define('LOCAL_ATT_VIEW_DAYS', 1);
define('LOCAL_ATT_VIEW_WEEKS', 2);
define('LOCAL_ATT_VIEW_MONTHS', 3);
define('LOCAL_ATT_VIEW_ALLPAST', 4);
define('LOCAL_ATT_VIEW_ALL', 5);
define('LOCAL_ATT_VIEW_NOTPRESENT', 6);
...
class local_attendance_permissions {
    ...
    public function can_view() {
        if (is_null($this->canview)) {
            $this->canview = has_capability('local/attendance:view', $this->context);
        }
    ...
    public function can_view_reports() {
        if (is_null($this->canviewreports)) {
            $this->canviewreports = has_capability('local/attendance:viewreports', $this->context);
        }
    ...
    public function require_view_reports_capability() {
        require_capability('local/attendance:viewreports', $this->context);
    }

    public function can_take() {
        if (is_null($this->cantake)) {
            $this->cantake = has_capability('local/attendance:takeattendances', $this->context);
        }
...
//        if ($groupid == local_attendance::SESSION_COMMON
...
//        }
    ...
    public function can_change() {
        if (is_null($this->canchange)) {
            $this->canchange = has_capability('local/attendance:changeattendances', $this->context);
        }
    ...
    public function can_manage() {
        if (is_null($this->canmanage)) {
            $this->canmanage = has_capability('local/attendance:manageattendances', $this->context);
        }
    ...
    public function require_manage_capability() {
        require_capability('local/attendance:manageattendances', $this->context);
    }

    public function can_change_preferences() {
        if (is_null($this->canchangepreferences)) {
            $this->canchangepreferences = has_capability('local/attendance:changepreferences', $this->context);
        }
    ...
    public function require_change_preferences_capability() {
        require_capability('local/attendance:changepreferences', $this->context);
    }

    public function can_export() {
        if (is_null($this->canexport)) {
            $this->canexport = has_capability('local/attendance:export', $this->context);
        }
    ...
    public function require_export_capability() {
        require_capability('local/attendance:export', $this->context);
    }

    public function can_be_listed() {
        if (is_null($this->canbelisted)) {
            $this->canbelisted = has_capability('local/attendance:canbelisted', $this->context, null, false);
        }
...
class local_att_page_with_filter_controls {
    ...
    public function init($sessioninfo) {
        $this->customsession = $sessioninfo;
        $this->init_view();
        $this->init_curdate();
    ...
    private function init_view() {
        ...
        if (isset($this->view)) {
            $SESSION->attcurrentattview[$this->customsession->id] = $this->view;
        } else if (isset($SESSION->attcurrentattview[$this->customsession->id])) {
            $this->view = $SESSION->attcurrentattview[$this->customsession->id];
        } else {
    ...
    private function init_curdate() {
        ...
        if (isset($this->curdate)) {
            $SESSION->attcurrentattdate[$this->customsession->id] = $this->curdate;
        } else if (isset($SESSION->attcurrentattdate[$this->customsession->id])) {
            $this->curdate = $SESSION->attcurrentattdate[$this->customsession->id];
        } else {
            $this->curdate = time();
        }
    }
...
//            $this->sessgroupslist[self::SESSTYPE_COMMON] = get_string('commonsessions', 'local_attendance');
...
class local_att_view_page_params extends local_att_page_with_filter_controls {
    ...
    public function __construct() {
        $this->defaultview = LOCAL_ATT_VIEW_MONTHS;
    }
...
class local_att_sessions_page_params {
...
class local_att_take_page_params {
...
class local_attendance {
    ...
    /* specific sessionacademicyear info */
    public $semesterinfo;
     ...
     */
    public function __construct(stdclass $dbrecord, $classinfo = null, stdclass $context = null, $pageparams = null) {
        global $DB, $CFG, $USER;
        ...
        if ($classinfo)
            ...
            $this->classinfo = $DB->get_record('local_clclasses', array('id' => $dbrecord->classid));

        $this->semesterinfo = $DB->get_record('local_semester', array('id' => $this->classinfo->semesterid));
    }
    ...
    public function get_hidden_sessions_count() {
        ...
        $params = array(
            'aid' => $this->id,
            // 'csdate'=> $this->sessionacademicyearinfo->startdate);
            'csdate' => $this->semesterinfo->startdate);
    ...
    public function url_manage($params = array()) {
        $params = array_merge(array('id' => $this->id), $params);
        return new moodle_url('/local/attendance/manage.php', $params);
    }
    ...
    public function add_sessions($sessions) {
        ...
        foreach ($sessions as $sess) {
            ...
            $sess->id = $DB->insert_record('local_attendance_sessions', $sess);
        ...
        foreach ($sessions as $sess) {
            if ($i > $maxlog) {
                ...
                $info_array[] = '...';
                $info_array[] = construct_local_session_full_date_time($lastsession->sessdate, $lastsession->duration);
                break;
            } else {
                $info_array[] = construct_local_session_full_date_time($sess->sessdate, $sess->duration);
            }
            $i++;
        }

        add_to_log($this->course->id, 'local_attendance', 'sessions added', $this->url_manage(), implode(',', $info_array), $this->id);
    }

    public function update_session_from_form_data($formdata, $sessionid) {
        global $DB;

        if (!$sess = $DB->get_record('local_attendance_sessions', array('id' => $sessionid))) {
...
//                                array('subdirs' => false, 'maxfiles' => -1, 'maxbytes' => 0), $formdata->sdescription['text']);
        ...
        $DB->update_record('local_attendance_sessions', $sess);

        $url = $this->url_sessions(array('sessionid' => $sessionid, 'action' => local_att_sessions_page_params::ACTION_UPDATE));
        $info = construct_local_session_full_date_time($sess->sessdate, $sess->duration);
        //add_to_log($this->course->id, 'local_attendance', 'session updated', $url, $info, $this->cm->id);
    }
    ...
    public function take_from_student($mformdata) {
        ...
        $record->remarks = get_string('set_by_student', 'local_attendance');
        ...
        if (array_key_exists($record->studentid, $dbsesslog)) {
            ...
            $DB->insert_record('local_attendance_log', $record, false);
        }
        ...
        $DB->update_record('local_attendance_sessions', $rec);
        ...
        $logurl = local_att_log_convert_url($url);

        // Log the change.
        //add_to_log($this->id, 'attendance', 'taken by student', $logurl, '', $this->cm->id);
    ...
    public function take_from_form_data($formdata) {
        ...
        foreach ($sesslog as $log) {
            if ($log->statusid) {
                if (array_key_exists($log->studentid, $dbsesslog)) {
                    ...
                    $DB->update_record('local_attendance_log', $log);
                } else {
                    $DB->insert_record('local_attendance_log', $log, false);
                }
            }
        }
        ...
        //// Log the change.
        //add_to_log($this->course->id, 'local_attendance', 'taken', $logurl, '', $this->cm->id);
        //
        //$group = 0;
        //if ($this->pageparams->grouptype != local_attendance::SESSION_COMMON) {
        ...
        $totalusers = $this->get_count_class_user();
        //count_enrolled_users(context_module::instance($this->cm->id), 'local/attendance:canbelisted', $group);
...
//}
    ...
    public function get_count_class_user() {

        global $DB, $CFG;

        $params = array($this->id);
        $sql = "select u.* from {local_attendance} as la 
...
// end of user
    ...
    public function get_users($groupid = 0, $page = 1) {
        ...
        if ($page) {
            ...
            //        $groups = $groupid;
            ////    }
            //    $users = get_users_by_capability($this->context, 'local/attendance:canbelisted',
            //                    $userfields.',u.id, u.firstname, u.lastname, u.email',
            //                    $orderby, $startusers, $usersperpage, $groups,
            //                    '', false, true);
            //} else {
            $startusers = ($page - 1) * $usersperpage;
            $users = $this->get_class_users($this->context, 'local/attendance:canbelisted', $groupid, $userfields, $orderby, $startusers, $usersperpage);
            // }
        } else {
            //if (!empty($CFG->enablegroupmembersonly) and $this->cm->groupmembersonly) {
            //    if ($groupid == 0) {
            ...
            //                    '', false, true);
            //} else {
            $users = $this->get_class_users($this->context, 'local/attendance:canbelisted', $groupid, $userfields, $orderby);
            // }
        }
        ...
        //    foreach ($users as $user) {
            ...
            foreach ($users as $user) {
                if (isset($user->semesterid))
                    $semesterinfo = $DB->get_record('local_semester', array('id' => $user->semesterid));
                $status = ($user->registrarapproval == 1 ? 0 : 1);
                $users[$user->id]->enrolmentstatus = $status;
                $users[$user->id]->enrolmentstart = $semesterinfo->startdate;
                $users[$user->id]->enrolmentend = $semesterinfo->enddate;
            }
        }
        return $users;
    }
    ...
    public function get_session_info($sessionid) {
        ...
        if (isset($this->sessioninfo)) {
            if (!array_key_exists($sessionid, $this->sessioninfo)) {
                $this->sessioninfo[$sessionid] = $DB->get_record('local_attendance_sessions', array('id' => $sessionid));
            }
...
//                        'pluginfile.php', $this->context->id, 'local_attendance', 'session', $this->sessioninfo[$sessionid]->id);
            //       }
    ...
    public function get_sessions_info($sessionids) {
        ...
        $sessions = $DB->get_records_select('local_attendance_sessions', "id $sql", $params, 'sessdate asc');
...
//            }
    ...
    public function get_user_taken_sessions_count($userid) {
        if (!array_key_exists($userid, $this->usertakensesscount)) {
            if (!empty($this->pageparams->startdate) && !empty($this->pageparams->enddate)) {
                $this->usertakensesscount[$userid] = local_att_get_user_taken_sessions_count($this->id, $this->semesterinfo->startdate, $userid, $this->id, $this->pageparams->startdate, $this->pageparams->enddate);
            } else {
                $this->usertakensesscount[$userid] = local_att_get_user_taken_sessions_count($this->id, $this->semesterinfo->startdate, $userid, $this->id);
            }
        }
    ...
    public function get_user_statuses_stat($userid, array $filters = null) {
        ...
        $params = array(
            'aid' => $this->id,
            'cstartdate' => $this->semesterinfo->startdate,
            'uid' => $userid);
...
//        } else {
        $qry = "SELECT al.statusid, count(al.statusid) AS stcnt
                  FROM {local_attendance_log} al
                  JOIN {local_attendance_sessions} ats
    ...
    public function get_user_grade($userid, array $filters = null) {
        return local_att_get_user_grade($this->get_user_statuses_stat($userid, $filters), $this->get_statuses());
    }
    ...
    public function update_users_grade($userids) {
        ...
        foreach ($userids as $userid) {
            ...
            $grades[$userid]->rawgrade = local_att_calc_user_grade_fraction($this->get_user_grade($userid), $this->get_user_max_grade($userid)) * $this->grade;
        }
...
//        } else {
        $sql = "SELECT ats.id, ats.sessdate, ats.groupid, al.statusid, al.remarks
                  FROM {local_attendance_sessions} ats
                  JOIN {local_attendance_log} al
...
//            }
    ...
    public function delete_sessions($sessionsids) {
        ...
        $DB->delete_records_list('local_attendance_sessions', 'id', $sessionsids);
    ...
    public function update_sessions_duration($sessionsids, $duration) {
        ...
        $now = time();
        $sessions = $DB->get_recordset_list('local_attendance_sessions', 'id', $sessionsids);
        foreach ($sessions as $sess) {
    ...
    public function remove_status($statusid) {
        ...
        $DB->set_field('local_attendance_statuses', 'deleted', 1, array('id' => $statusid));
    }
    ...
    public function add_status($acronym, $description, $grade) {
        ...
        if ($acronym && $description) {
            $rec = new stdClass();
            $rec->livecourseid = $this->livecoursemappinginfo->livecourseid;
            ...
            add_to_log($this->course->id, 'local_attendance', 'status added', $this->url_preferences(), $acronym . ': ' . $description . ' (' . $grade . ')', $this->cm->id);
        } else {
            print_error('cantaddstatus', 'local_attendance', $this->url_preferences());
        }
    }
    ...
    public function update_status($statusid, $acronym, $description, $grade, $visible) {
        ...
        $DB->update_record('local_attendance_statuses', $status);

        add_to_log($this->classinfo->id, 'local_attendance', 'status updated', $this->url_preferences(), implode(' ', $updated), $this->semesterinfo->id);
    }

}

function local_att_get_statuses($attid, $onlyvisible = true) {
    global $DB;
    if ($onlyvisible) {
        $statuses = $DB->get_records_select('local_attendance_statuses', "attendanceid = :aid AND visible = 1 AND deleted = 0", array('aid' => $attid), 'grade DESC');
    } else {
        $statuses = $DB->get_records_select('local_attendance_statuses', "attendanceid = :aid AND deleted = 0", array('aid' => $attid), 'grade DESC');
    }

    return $statuses;
}

function local_att_get_user_taken_sessions_count($attid, $sessionacademicyearstartdate, $userid, $coursemodule, $startdate = '', $enddate = '') {
    global $DB, $COURSE;
//    $groupmode = groups_get_activity_groupmode($coursemodule, $COURSE);
//    if (!empty($groupmode)) {
//        $qry = "SELECT count(*) as cnt
//              FROM {local_attendance_log} al
//              JOIN {local_attendance_sessions} ats ON al.sessionid = ats.id
//              LEFT JOIN {groups_members} gm ON gm.userid = al.studentid AND gm.groupid = ats.groupid
//             WHERE ats.attendanceid = :aid AND
//                   ats.sessdate >= :cstartdate AND
//                   al.studentid = :uid AND
...
//    } else {
    ...
    $params = array(
        'aid' => $attid,
        'cstartdate' => $sessionacademicyearstartdate,
        'uid' => $userid);
...
function local_att_get_user_statuses_stat($attid, $coursestartdate, $userid, $coursemodule) {
    global $DB, $COURSE;
//    $groupmode = groups_get_activity_groupmode($coursemodule, $COURSE);
//    if (!empty($groupmode)) {
//        $qry = "SELECT al.statusid, count(al.statusid) AS stcnt
...
function local_att_get_user_grade($userstatusesstat, $statuses) {
    $sum = 0;
...
function local_att_get_user_max_grade($sesscount, $statuses) {
    reset($statuses);
    return current($statuses)->grade * $sesscount;
}

function local_att_get_user_courses_attendances($userid) {
    global $DB;
    ...
    $sql = "SELECT att.id as attid, att.course as courseid, course.fullname as coursefullname,
              ...
              FROM {local_attendance} att
              JOIN {local_course} course
...
function local_att_calc_user_grade_fraction($grade, $maxgrade) {
    if ($maxgrade == 0) {
        return 0;
    } else {
...
function local_att_get_gradebook_maxgrade($attid) {
    global $DB;

    return $DB->get_field('local_attendance', 'grade', array('id' => $attid));
}
...
function local_att_has_logs_for_status($statusid) {
    global $DB;

    return $DB->count_records('local_attendance_log', array('statusid' => $statusid)) > 0;
}

function local_att_log_convert_url(moodle_url $fullurl) {
    static $baseurl;

    if (!isset($baseurl)) {
        $baseurl = new moodle_url('/local/attendance/');
        $baseurl = $baseurl->out();
    }
...
function local_attforblock_upgrade() {
    global $DB, $CFG;
    ...
    $dbman = $DB->get_manager(); // Loads ddl manager and xmldb clclasses.
