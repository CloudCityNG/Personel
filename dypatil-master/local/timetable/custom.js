     ...
     /*   eventRender: function (event, element) {
            ...
            element.click(function () {
                ...
                if (event.attendance) {                 
                    if (event.today <= 0) {
                        if (event.today ==0 ) {
                           des += '<b><a href= "' + M.cfg.wwwroot + '/local/attendance/take.php?classid=' + event.id + '&time=' + event.starttime + '">take attendance<img src=' + M.cfg.wwwroot + '/local/attendance/pix/icon.gif' + '></img></a> ';  
                        }
                        else{
                         if (event.today < 0 )   
                        des += '<b><a href= "' + M.cfg.wwwroot + '/local/attendance/take.php?classid=' + event.id + '&time=' + event.starttime + '">Edit attendance<img src=' + M.cfg.wwwroot + '/local/attendance/pix/icon.gif' + '></img></a> ';
                       }
                    }
                    else
