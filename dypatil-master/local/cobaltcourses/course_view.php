...
$PAGE->set_url('/course/course_view.php');
...
$authorintro = get_string('authorintronavbar', 'local_cobaltcourses');
...
<div class="heading">
            ...
            if ($users = get_role_users($roleid, $context, true)) {
                foreach ($users as $teacher) {
                    ...
                    if (empty($user->description)) {
                        $user->description = '<p><span>' . get_string('nodes', 'local_cobaltcourses') . '</span></p>';
                    }
    ...
    <div class="course_sum" >
        <h3><?php echo get_string('coursesum', 'local_cobaltcourses'); ?> </h3>
        <?php
        $fs = get_file_storage();
...
WHERE u.id=r.userid AND r.contextid = {$context->id} and u.id={$USER->id}";
    ...
    if (isloggedin()) {
        if (empty($enrolled_user) AND ! is_siteadmin()) {
            echo'<div  class="enroll"><a href="../../enrol/index.php?id=' . $id . '"><button>' . get_string('enrollcourse', 'local_cobaltcourses') . '</button></a></div>';
        } else {
            echo'<div  class="enroll"><a href="../../course/view.php?id=' . $id . '"><button>' . get_string('gotocourse', 'local_cobaltcourses') . '</button></a></div>';
        }
    } else {
        echo'<div   class="enroll"><a href="../../login/index.php?id=' . $id . '"><button>' . get_string('enrollcourse', 'local_cobaltcourses') . '</button></a></div>';
    }
    ?>
</div>
<div class="syllabus">
    <h3><?php echo get_string('authorintro', 'local_cobaltcourses'); ?> </h3>
