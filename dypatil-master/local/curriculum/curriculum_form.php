...
class makeactiveplan_form extends moodleform {
    ...
    function definition() {
        ...
        $PAGE->requires->yui_module('moodle-local_curriculum-makeactiveplan', 'M.local_curriculum.init_makeactiveplan', array(array('formid' => $mform->getAttribute('id'))));
        $mform->addElement('header', 'settingsheader', get_string('makeactiveplan', 'local_curriculum'));
        ...
        //No Submit button for yui
        $mform->registerNoSubmitButton('updatecourseformat');
        $mform->addElement('submit', 'updatecourseformat', get_string('courseformatudpate'));
        
        // Add programs dropdown here
        $mform->addElement('hidden', 'addprogramslisthere');
        $mform->setType('addprogramslisthere', PARAM_RAW);
        ...
        $this->add_action_buttons($cancel = true, get_string('makeactive', 'local_curriculum'));
    }
    ...
    function definition_after_data() {
        ...
        if (isset($schoolvalue) && $schoolvalue[0] > 0) {
            ...
            $tools = $hierarchy->get_records_cobaltselect_menu('local_program', "schoolid=$schoolvalue AND visible=1", null, '', 'id,fullname', '--Select--');

            $pgm = $mform->createElement('select', 'programid', get_string('program', 'local_programs'), $tools);
            $mform->insertElementBefore($pgm, 'addprogramslisthere');
            $mform->addRule('programid', get_string('required'), 'required', null, 'client');
            $mform->setType('programid', PARAM_INT);
            $programvalue = $mform->getElementValue('programid');
        }
        
        if (!empty($programvalue) && $programvalue[0] > 0) {
            $batches = $cplan->get_batches($schoolvalue, $programvalue[0]);
            $batch = $mform->createElement('select', 'batchid', get_string('selectbatch', 'local_curriculum'), $batches);
            $mform->insertElementBefore($batch, 'addprogramslisthere');
            $mform->addRule('batchid', get_string('required'), 'required', null, 'client');
            $mform->setType('batchid', PARAM_INT);
            $batchvalue = $mform->getElementValue('batchid');
        }
        
        if (!empty($batchvalue) && $batchvalue[0] > 0) {
            
            $semesters = $cplan->get_active_semesters($schoolvalue);
            $sem = $mform->createElement('select', 'semesterid', get_string('selectsemester', 'local_curriculum'), $semesters);
            $mform->insertElementBefore($sem, 'addprogramslisthere');
            ...
            //if (!empty($semestervalue) && $semestervalue[0] > 0) {
            //    $exist = $DB->get_record('local_activeplan_batch', array('batchid'=>$batchvalue[0], 'semesterid'=>$semestervalue[0]));
            //    if($exist){
            //        $seminfo = $mform->createElement('static', 'seminfo', '', get_string('seminfo', 'local_curriculum'));
            //        $mform->insertElementBefore($seminfo, 'addprogramslisthere');
            //    }
            //}
            
            $plans = $cplan->get_batch_plans($schoolvalue, $programvalue[0], $batchvalue[0]);
            $plan = $mform->createElement('select', 'planid', get_string('selectplan', 'local_curriculum'), $plans);
            $mform->insertElementBefore($plan, 'addprogramslisthere');
            $mform->addHelpButton('planid', 'selectplan', 'local_curriculum');
            $mform->addRule('planid', get_string('required'), 'required', null, 'client');
            $mform->setType('planid', PARAM_INT);
            
            $planvalue = $mform->getElementValue('planid');
        }
    }
    ...
    public function validation($data, $files) {
        ...
        $record = $DB->get_record('local_activeplan_batch', array('semesterid'=>$data->semesterid, 'batchid'=>$data->batchid));
        if($record){
            $errors['batchid'] = get_string('semmadeactive', 'local_curriculum');
        }
        $record_exist = $DB->get_record('local_activeplan_batch', array('planid'=>$data->planid, 'batchid'=>$data->batchid));
        if($record_exist){
            $errors['planid'] = get_string('planactiveforotheroffering', 'local_curriculum');
        }
        return $errors;
    }
}