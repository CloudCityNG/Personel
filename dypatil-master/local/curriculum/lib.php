...
//     $toprow[] = new tabobject('setting', new moodle_url('/local/curriculum/setting.php'), get_string('curriculumsettings','local_curriculum'));
        ...
        $toprow[] = new tabobject('activeplan', new moodle_url('/local/curriculum/activeplan.php'), get_string('activeplan','local_curriculum'));
...
class curriculumplan {
    ...
    function display_curriculum_plan($plan) {
        ...
        if(has_capability('local/collegestructure:manage', $context)){
            ...
            $out .= '<b>'.format_string($plan->fullname).'</b>';
        }
        
        $out .= html_writer::end_tag('div');

        $sql = "SELECT cc.* FROM {local_curriculum_plancourses} cp,{local_cobaltcourses} cc where cp.planid={$plan->id} AND cp.courseid=cc.id";
        ...
        foreach ($plancourses as $plancourse) {
            ...
            if(has_capability('local/collegestructure:manage', $context)){
                ...
                $out .= html_writer::start_tag('div', array('class' => 'plan ' . $itemdepth12 ));
                $plancourse->summary = $plancourse->summary != '' ? $plancourse->summary : 'No information to show' ;
                $desc = html_writer::tag('div', $plancourse->summary, array('class'=>'coursedescription'));
                $coursename = html_writer::tag('a', format_string($plancourse->fullname), array('href'=>'javascript:void(0)', 'class'=>'collapsed'));
                $out .= html_writer::tag('div', $coursename . $desc, array('class'=>'semestercourse'));
    ...
    function get_batches($schoolid, $programid){
        global $DB, $CFG;
        $sql = "SELECT batch.* FROM {local_batch_map} AS map
                    JOIN {cohort} AS batch ON batch.id = map.batchid
                    WHERE map.schoolid = {$schoolid} AND map.programid = {$programid}
                    AND batch.visible = 1";
        $records = $DB->get_records_sql($sql);
        $batches = array(null=>'---Select---');
        if(empty($records)){
            return $batches;
        }
        foreach($records as $record){
            $batches[$record->id] = format_string($record->name);
        }
        return $batches;
    }
    
    function get_batch_plans($schoolid, $programid, $batchid){
        global $DB, $CFG;
        $sql = "SELECT plan.* FROM {local_curriculum_plan} AS plan
                JOIN {local_batch_map} AS map ON map.curriculumid = plan.curriculumid
                WHERE map.schoolid = {$schoolid} AND
                map.programid = {$programid} AND map.batchid = {$batchid}
                AND plan.visible = 1";
                
        $records = $DB->get_records_sql($sql);
        $plans = array(null=>'---Select---');
        if(empty($records)){
            return $plans;
        }
        foreach($records as $record){
            $plans[$record->id] = format_string($record->fullname);
        }
        return $plans;
    }
    ...
    public function get_active_semesters($id) {
        ...
        // Fetching all current and upcoming semesters based on present date
