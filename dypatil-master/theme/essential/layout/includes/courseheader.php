...
if(!$school = $DB->get_record_sql('select s.* from {local_school} as s LEFT JOIN {local_userdata} as ud ON s.id=ud.schoolid where ud.userid='.$USER->id.''))
$school = $DB->get_record_sql('select s.* from {local_school} as s LEFT JOIN {local_school_permissions} as lu ON s.id=lu.schoolid where lu.userid='.$USER->id.'');


$contextid = context_system::instance();
$fs = get_file_storage();

$files = $fs->get_area_files($contextid->id, 'theme_'.$school->theme.'', 'logo', $school->id);

foreach ($files as $file) {
    $filename = $file->get_filename();

}
 $logo = $CFG->wwwroot.'/pluginfile.php/'.$contextid->id.'/theme_'.$school->theme.'/logo/'.$school->id.'/'.rawurlencode($filename).'';
 $logo = "$CFG->wwwroot/pluginfile.php/$contextid->id/theme_$school->theme/logo/$school->id/".$filename."";
if($filename)
echo '<a class="navbar-brand" href='.$CFG->wwwroot.'><img src='.$logo.'></a>';
else
echo '<a class="navbar-brand" href='.$CFG->wwwroot.'><img src='.$OUTPUT->pix_url('c4link_big','theme_essential').'></a>';
    ...
    <nav role="navigation">
        <div id='essentialnavbar' class="navbar<?php echo ($oldnavbar) ? ' oldnavbar' : ''; ?> moodle-has-zindex">
                                <div class="container-fluid navbar-inner" id="user-area">
                                      <?php
                                                $systemcontext = context_system::instance();
              $context = context_user::instance($USER->id);
    //for instructor starts-------------------------------------------------------
         echo "<span id='editingon'>".$OUTPUT->page_heading_button()."</span>";
    

 ?>                                                   <div class="usermenu">
                            <?php
                       
                            echo $OUTPUT->custom_menu_user();
