...
class theme_essential_core_renderer extends core_renderer {
    ...
    public function custom_menu_messages() {
        ...
        if (empty($totalmessages)) {
            ...
            $messagemenutext = $messagemenuicon . html_writer::tag('sup', $messages['newmessages']);
            $messagemenu->add(
                $messagemenutext,
     ...
     * @return custom_menu object
     */
    public function cobalt_applications() {
        global $CFG, $USER;
        $application_menu = new custom_menu();
        ...
        $content = '<ul class="nav">';
        if (has_capability('local/collegestructure:manage', context_system::instance()) && !is_siteadmin($USER->id)) {
            $content .="<li class='dropdown'>
                            <a class='dropdown-toggle' title='Applications' data-toggle='dropdown' href='#'>
                            <i class='fa fa-envelope-o'></i>
                            <sup id='requests'>$applicationnotice</sup>
    ...
    public function custom_menu_user() {
        ...
        if (!isloggedin()) {
            ...
            $usermenu .= html_writer::link($userurl, $userpic . /*'<div>'.*/$USER->firstname . $caret/*.'</div>'*/, $userclass);
    ...
    */
    ...
    public function render_slide_controls($left) {
        ...
        $prev = '<a class="left carousel-control" href="#essentialCarousel" data-slide="prev"><i class="fa fa-angle-' . $faleft . '"></i></a>';
        $next = '<a class="right carousel-control" href="#essentialCarousel" data-slide="next"><i class="fa fa-angle-' . $faright . '"></i></a>';
    ...
    public function get_title($location) {
        ...
        $title = '';
        $home_icon = $OUTPUT->pix_url('Home', 'theme');
        if ($location === 'navbar') {
            ...
            switch ($this->get_setting('navbartitle')) {
                ...
                break;
                case 1:
                    $title = format_string($SITE->fullname, true,
                                    array('context' => context_course::instance(SITEID)));
                    $title = '<a class="brand" href="' . $url . '">' . '<img src="'.$home_icon.'" alt="'.$title.'" />' . '</a>';
                    break;
                case 2:
                    $title = format_string($SITE->shortname, true,
                                    array('context' => context_course::instance(SITEID)));
                    $title = '<a class="brand" href="' . $url . '">' . '<img src="'.$home_icon.'" alt="'.$title.'" />' . '</a>';
                    break;
                default:
